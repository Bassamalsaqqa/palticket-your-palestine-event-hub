...
export class OpsService {
  ...
  async createPosOrder(
    ...
    if (paymentMethod === PaymentMethod.CARD && !providerReference) {
      throw new BadRequestException('Provider reference required for CARD payments');
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const ticketTypeIds = [...new Set(items.map((i) => i.ticketTypeId))].sort();
      ...
      const inventoriesRaw: any[] = await tx.$queryRawUnsafe(query, ...ticketTypeIds);
      ...
      for (const item of items) {
        const inventory = inventories.find(inv => inv.ticketTypeId === item.ticketTypeId);
        const typeInfo = ticketTypes.find(t => t.id === item.ticketTypeId);

        if (!inventory || !typeInfo) {
           throw new BadRequestException(`Invalid ticket type: ${item.ticketTypeId}`);
        }

        if (typeInfo.currency !== currency) {
           throw new BadRequestException(`Currency mismatch for ticket type ${item.ticketTypeId}`);
        }

        if (inventory.sold + inventory.reserved + item.quantity > inventory.capacity) {
          throw new BadRequestException(`Insufficient capacity for ticket type ${item.ticketTypeId}`);
        }
      ...
      const orderStatus = isPaid ? OrderStatus.PAID : OrderStatus.PENDING_PAYMENT;
      const paymentStatus = isPaid ? PaymentStatus.SUCCEEDED : PaymentStatus.PENDING;
      ...
      const order = await tx.order.create({
        data: {
          ...
          paymentProvider: paymentMethod === PaymentMethod.CARD ? 'manual' : null,
